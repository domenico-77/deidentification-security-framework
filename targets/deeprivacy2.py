...
class DeepPrivacy2Target(BaseDeidentificationTarget):
    def __init__(self, config_path: str = "stylegan_fdf128", models_dir: str = None):
        self.generator = self._load_generator(config_path, models_dir)

    def _load_generator(self, config_path, models_dir):
        ...
        # Usiamo la configurazione diretta del generatore (stylegan_fdf128.py) invece dell'anonymizer video
        if config_path is None or config_path in ["fdf128", "face_fdf128"]:
            config_path = "stylegan_fdf128"
    ...
    def process_image(self, image: torch.Tensor, *args, **kwargs) -> np.ndarray:
        """
        Elabora la singola immagine tramite il generatore di DeepPrivacy2.
        """
        ...
        with torch.no_grad():
            anonymized_img = self.generator(img_np)
